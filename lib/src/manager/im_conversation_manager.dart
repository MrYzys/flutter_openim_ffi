...
class ConversationManager {
  ...
  Future hideAllConversations({String? operationID}) => _channel.invokeMethod(
  ...
  Future<List<dynamic>> getConversationRecvMessageOpt({
    ...
    final idSet = conversationIDList.toSet();
    return conversations
        .where((info) => idSet.contains(info.conversationID))
        .map(
          (info) => {
            'conversationID': info.conversationID,
            'recvMsgOpt': info.recvMsgOpt ?? 0,
          },
        )
        .toList();
  }
  ...
  Future<void> deleteAllConversationFromLocal({String? operationID}) async {
    final conversations = await getAllConversationList(
      operationID: operationID,
    );
    if (conversations.isEmpty) return;
    for (final item in conversations) {
      try {
        await deleteConversationAndDeleteAllMsg(
        ...
        try {
          ...
          // Ignore individual failures; proceed with remaining conversations.
        }
      }
    }
  }
  ...
  /// Query @ All Flag
  Future<dynamic> getAtAllTag({String? operationID}) => _channel.invokeMethod(
    'getAtAllTag',
    _buildParam({"operationID": Utils.checkOperationID(operationID)}),
  );
